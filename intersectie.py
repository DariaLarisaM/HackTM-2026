...
def trimite_alerta_api(id_obiect, viteza, scor):
    ...
    try:
        # requests.post(API_URL, json=payload, timeout=2) # Decomentează când ai API-ul pornit
...
while cap.isOpened():
    ...
    # Folosim .track() pentru a urmări obiectele de la un cadru la altul
    ...
    for r in results:
        ...
        # Extragem ID-urile dacă modelul a reușit să le urmărească
        if boxes.id is not None:
            track_ids = boxes.id.int().cpu().tolist()
        else:
            track_ids = [-1] * len(boxes) # ID de rezervă dacă tracking-ul eșuează temporar

        for box, track_id in zip(boxes, track_ids):
            x1, y1, x2, y2 = int(box.xyxy[0][0]), int(box.xyxy[0][1]), int(box.xyxy[0][2]), int(box.xyxy[0][3])
            center_x, center_y = int((x1 + x2) / 2), int((y1 + y2) / 2)

            # --- 1. Logica de numărare pentru semafor ---
            ...
            # --- 2. Logica de viteză și detecție accidente (doar pentru obiecte urmărite) ---
            if track_id != -1:
                ...
                if len(istoric) >= 5:
                    ...
                    if timp_scurs > 0:
                        ...
                        viteza_estimata = viteza_pixeli_sec * 0.15 # Multiplicator pentru machetă
                        
                        viteza_anterioara = viteze_curente.get(track_id, viteza_estimata)
                        
                        # Calcul probabilitate accident (frânare extremă/oprire bruscă)
                        if viteza_anterioara > 30 and viteza_estimata < 5:
                            scor_accident = 85.0 
                        elif viteza_anterioara > 15 and viteza_estimata < 2:
                            scor_accident = 50.0 
                            
                        # Declanșare API dacă e periculos
                        if scor_accident > 40:
    ...
    # --- LOGICA HIBRIDĂ SEMAFOR (TIMP + AI) ---
    ...
    # Comunicare Arduino cu noile semnale
